...
class TradingBot:
    ...
    async def handle_channel_message(self, event):
        ...
        try:
            ...
            if not message:
                logging.info(f"Invalid message-IGNORE")
                return
    ...
    async def check_signal_status(self, signal: Dict[str, Any]):
        ...
        try:
            ...
            if signal.get('entry_zones'):
                try:
                    entry_zones = json.loads(signal['entry_zones']) if isinstance(signal['entry_zones'], str) else \
                    signal['entry_zones']
                    # 使用字典访问而不是对象属性
                    filled_zones = sum(
                        1 for zone in entry_zones if isinstance(zone, dict) and zone.get('status') == 'FILLED')
    ...
    async def generate_statistics(self) -> dict:
        ...
        try:
            ...
            weekly_pnl = sum(
                t['pnl'] for t in trades if t['close_time'].date() >= (datetime.now() - timedelta(days=7)).date())
            monthly_pnl = sum(
                t['pnl'] for t in trades if t['close_time'].date() >= (datetime.now() - timedelta(days=30)).date())
